...
exports.default = {
  ...
  mounted: function mounted() {
    this.date.time = (0, _momentJalaali2.default)(this.value).format(this.option.format);
  },
  ...
  methods: {
    ...
    nextMonth: function nextMonth(type) {
      var next = null;
      type === 'next' ? next = (0, _momentJalaali2.default)(this.checked.currentMoment).add(1, 'jMonth') : next = (0, _momentJalaali2.default)(this.checked.currentMoment).add(-1, 'jMonth');
      this.showDay(next);
    },
    today: function today() {
      this.showDay((0, _momentJalaali2.default)(Date.now()));
    },
    showDay: function showDay(time) {
      if (time === undefined || !Date.parse(time)) {
        this.checked.currentMoment = (0, _momentJalaali2.default)();
      } else {
        if ((typeof time === 'undefined' ? 'undefined' : _typeof(time)) === 'object') {
          this.checked.currentMoment = (0, _momentJalaali2.default)(time);
        } else {
          this.checked.currentMoment = (0, _momentJalaali2.default)(time, this.option.format);
        }
      }
      this.showOne('day');
      this.checked.year = (0, _momentJalaali2.default)(this.checked.currentMoment).format('jYYYY');
      this.checked.month = (0, _momentJalaali2.default)(this.checked.currentMoment).format('jMM');
      this.checked.day = (0, _momentJalaali2.default)(this.checked.currentMoment).format('jDD');
      this.displayInfo.month = this.library.month[(0, _momentJalaali2.default)(this.checked.currentMoment).jMonth()];
      var days = [];
      var currentMoment = this.checked.currentMoment;
      var firstDay = (0, _momentJalaali2.default)(currentMoment).jDate(1).day() + 2;
      // gettting previous and next month
      // let currentMonth = moment(currentMoment)
      var previousMonth = (0, _momentJalaali2.default)(currentMoment);
      var nextMonth = (0, _momentJalaali2.default)(currentMoment);
      nextMonth.add(1, 'jMonth');
      previousMonth.subtract(1, 'jMonth');
      var monthDays = parseInt((0, _momentJalaali2.default)(currentMoment).endOf('jMonth').jDate());
      ...
      for (var i = 1; i <= monthDays; ++i) {
        days.push({
          ...
          unavailable: false,
          checked: false,
          moment: (0, _momentJalaali2.default)(currentMoment).jDate(i)
        });
        if (i === Math.ceil((0, _momentJalaali2.default)(currentMoment).format('jD')) && (0, _momentJalaali2.default)(oldtime, this.option.format).jYear() === (0, _momentJalaali2.default)(currentMoment).jYear() && (0, _momentJalaali2.default)(oldtime, this.option.format).jMonth() === (0, _momentJalaali2.default)(currentMoment).jMonth()) {
      ...
      for (var _i = 0; _i < firstDay - (this.option.SundayFirst ? 0 : 1); _i++) {
        var passiveDay = {
          value: parseInt(previousMonth.endOf('jMonth').jDate()) - _i,
          inMonth: false,
          action: 'previous',
          unavailable: false,
          checked: false,
          moment: (0, _momentJalaali2.default)(currentMoment).jDate(1).subtract(_i + 1, 'jDays')
        };
      ...
      for (var _i2 = 1; _i2 <= passiveDaysAtFinal; _i2++) {
        var _passiveDay = {
          ...
          unavailable: false,
          checked: false,
          moment: (0, _momentJalaali2.default)(currentMoment).add(1, 'jMonths').jDate(_i2)
        };
    ...
    checkBySelectDays: function checkBySelectDays(d, days) {
      ...
      this.selectedDays.forEach(function (day) {
        if (_this.checked.year === (0, _momentJalaali2.default)(day).format('jYYYY') && _this.checked.month === (0, _momentJalaali2.default)(day).format('jMM') && d === Math.ceil((0, _momentJalaali2.default)(day).format('jD'))) {
    ...
    limitWeekDay: function limitWeekDay(limit, days) {
      days.map(function (day, index) {
        if (limit.available.indexOf(index % 7 + 1) === -1) {
    ...
    getLimitCondition: function getLimitCondition(limit, day) {
      var tmpMoment = (0, _momentJalaali2.default)(day.moment.year() + '-' + this.pad(day.moment.month() + 1) + '-' + this.pad(day.value));
      var tmpFrom = limit.from ? (0, _momentJalaali2.default)(limit.from, 'jYYYY-jMM-jDD') : null;
      var tmpTo = limit.to ? (0, _momentJalaali2.default)(limit.to, 'jYYYY-jMM-jDD') : null;
      if (tmpFrom && !tmpTo) {
        return !tmpMoment.isAfter(tmpFrom);
      } else if (!tmpFrom && tmpTo) {
        return !tmpMoment.isBefore(tmpTo);
      } else {
        return !tmpMoment.isBetween(tmpFrom, tmpTo);
      }
    },
    ...
    showYear: function showYear() {
      ...
      var year = (0, _momentJalaali2.default)(this.checked.currentMoment).jYear();
      this.library.year = [];
      var yearTmp = [];
    ...
    setYear: function setYear(year) {
      this.checked.currentMoment = (0, _momentJalaali2.default)(year + '-' + this.checked.month + '-' + this.checked.day, 'jYYYY-jMM-jD');
    ...
    setMonth: function setMonth(month) {
      ...
      this.checked.currentMoment = (0, _momentJalaali2.default)(this.checked.year + '-' + mo + '-' + this.checked.day, 'jYYYY-jMM-jD');
      this.showDay(this.checked.currentMoment);
    },
    ...
    picked: function picked() {
      if (this.option.type === 'day' || this.option.type === 'min') {
        ...
        this.checked.currentMoment = (0, _momentJalaali2.default)(ctime, 'jYYYY-jMM-jDD HH:mm');
        this.date.time = (0, _momentJalaali2.default)(this.checked.currentMoment).format(this.option.format);
      } else {
        this.date.time = JSON.stringify(this.selectedDays);
      }
      this.showInfo.check = false;
      this.$emit('change', this.date.time);
      this.$emit('input', this.checked.currentMoment);
    },
