...
export default {
  ...
  mounted () {
    this.date.time = moment(this.value).format(this.option.format)
  },
  ...
  methods: {
    ...
    nextMonth (type) {
      let next = null
      type === 'next' ? next = moment(this.checked.currentMoment).add(1, 'jMonth') : next = moment(this.checked.currentMoment).add(-1, 'jMonth')
      this.showDay(next)
    },

    today () {
      this.showDay(moment(Date.now()))
    },
    ...
    showDay (time) {
      ...
      this.showOne('day')
      this.checked.year = moment(this.checked.currentMoment).format('jYYYY')
      this.checked.month = moment(this.checked.currentMoment).format('jMM')
      this.checked.day = moment(this.checked.currentMoment).format('jDD')
      this.displayInfo.month = this.library.month[moment(this.checked.currentMoment).jMonth()]
      let days = []
      let currentMoment = this.checked.currentMoment
      let firstDay = moment(currentMoment).jDate(1).day() + 2
      ...
      let monthDays = parseInt(moment(currentMoment).endOf('jMonth').jDate())
      ...
      for (let i = 1; i <= monthDays; ++i) {
        days.push({
          ...
          unavailable: false,
          checked: false,
          moment: moment(currentMoment).jDate(i)
        })
        if (i === Math.ceil(moment(currentMoment).format('jD')) && moment(oldtime, this.option.format).jYear() === moment(currentMoment).jYear() && moment(oldtime, this.option.format).jMonth() === moment(currentMoment).jMonth()) {
      ...
      for (let i = 0; i < firstDay - (this.option.SundayFirst ? 0 : 1); i++) {
        let passiveDay = {
          value: parseInt(previousMonth.endOf('jMonth').jDate()) - (i),
          inMonth: false,
          action: 'previous',
          unavailable: false,
          checked: false,
          moment: moment(currentMoment).jDate(1).subtract(i + 1, 'jDays')
        }
      ...
      for (let i = 1; i <= passiveDaysAtFinal; i++) {
        let passiveDay = {
          ...
          unavailable: false,
          checked: false,
          moment: moment(currentMoment).add(1, 'jMonths').jDate(i)
        }
    ...
    checkBySelectDays (d, days) {
      this.selectedDays.forEach(day => {
        if (this.checked.year === moment(day).format('jYYYY') && this.checked.month === moment(day).format('jMM') && d === Math.ceil(moment(day).format('jD'))) {
    ...
    getLimitCondition (limit, day) {
      ...
      let tmpFrom = limit.from ? moment(limit.from, 'jYYYY-jMM-jDD') : null
      let tmpTo = limit.to ? moment(limit.to, 'jYYYY-jMM-jDD') : null
      if (tmpFrom && !tmpTo) {
    ...
    showYear () {
      let year = moment(this.checked.currentMoment).jYear()
      this.library.year = []
      let yearTmp = []
    ...
    setYear (year) {
      this.checked.currentMoment = moment(year + '-' + this.checked.month + '-' + this.checked.day, 'jYYYY-jMM-jD')
    ...
    setMonth (month) {
      ...
      this.checked.currentMoment = moment(this.checked.year + '-' + mo + '-' + this.checked.day, 'jYYYY-jMM-jD')
      this.showDay(this.checked.currentMoment)
    },
    ...
    picked () {
      if (this.option.type === 'day' || this.option.type === 'min') {
        ...
        this.checked.currentMoment = moment(ctime, 'jYYYY-jMM-jDD HH:mm')
